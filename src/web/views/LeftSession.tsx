...
export function LeftSession({ state, now, awaySince, elsewhere, onWatchAgain, onEndForReal, onSwitchTo }: LeftSessionProps) {
  ...
  const secondaryId = state.sessionName ? state.teamName : undefined;
      ...
      <div style={{ width: 560, maxWidth: '100%', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        >
          <div style={{ display: 'flex', gap: 9, alignItems: 'baseline' }}>
            ...
            <span style={{ color: 'var(--color-text)', fontSize: 12.5 }}>{title}</span>
            {secondaryId && (
              <span style={{ color: 'var(--color-neutral-600)', fontSize: 11, ...ELLIPSIS }}>{secondaryId}</span>
            )}
            <span style={{ flex: 1 }} />
